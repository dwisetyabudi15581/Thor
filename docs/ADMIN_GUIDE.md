...
- Di Discord, ketik `/` — semua **96 slash command** harus muncul
...
/set-channel server-booster #booster
```
...
Riwayat lengkap semua versi (v3.9.0 – v3.23.1) tersedia di **[CHANGELOG.md](../CHANGELOG.md)**.

Ringkasan semua versi:

- **v3.23.1** (2026-09-16) — 🔴 **FIX: HALAMAN RINGKASAN & AUTOMOD DI WEB CRASH UNTUK SERVER YANG BELUM MENGATUR AUTOMOD** (laporan pemilik: "This page couldn't load"). Akar: `automodManager.getGuildConfig()` return null untuk guild tanpa config tersimpan → payload `GET /guilds/:id/dashboard` mengirim `automod: null` → halaman web membaca `.enabled`/`.wordRules` dari null → exception di browser (hanya Overview & AutoMod yang membaca `draft.automod`; mock dev selalu kirim objek penuh sehingga tak tertangkap test). Fix: payload kirim fallback objek default (enabled=false, array kosong) + dashboard menormalkan payload pasca-fetch (pertahanan berlapis — aman meski bot belum diupdate). Test 735 → **736**.
