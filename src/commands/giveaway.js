    ...
    if (sub === 'list') {
        ...
        const lines = all
            .map(g => {
                ...
                const winnersStr =
                    g.ended && g.winnerIds.length > 0 ? g.winnerIds.map(id => `<@${id}>`).join(', ') : '—';
