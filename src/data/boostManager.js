/**
 * Boost Manager — riwayat & state server booster (v3.9.49).
 *
 ...
 *     guildId: "...",
 *     userId: "...",
 *     boostedAt: 1735689600000,   // mulai streak boost SAAT INI (null = belum pernah)
 *     removedAt: 1736000000000,   // kapan boost saat ini berakhir (null = masih boost / belum pernah)
 *     totalBoosts: 3,             // berapa kali user ini MULAI boost (sepanjang waktu)
 *     lastEventAt: 1736000000000, // timestamp event add/remove terakhir yang tercatat
 *     lastEvent: "add" | "remove"
 *   }
 * }
 *
 * Kenapa perlu file data padahal Discord tahu booster live?
 *   - premiumSinceTimestamp menjawab "siapa boost SEKARANG" tapi TIDAK "siapa
 *     yang boost saat bot offline" dan TIDAK "berapa banyak event boost".
 *   - Event yang datang adalah guildMemberUpdate dengan diff PREMIUM_SINCE —
 *     Discord tidak punya event boost khusus, jadi bot sendiri yang menurunkan
 *     add/remove dan menyimpan riwayatnya di sini.
 *
 * Pola: sama dengan modLogManager/warnManager — composite key
 * `${guildId}:${userId}`, safeWriteJSON + karantina file korup, scoped per guild.
 */
