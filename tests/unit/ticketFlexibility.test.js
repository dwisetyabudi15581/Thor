...
test('help.js: help embed mentions new v3.9.14+ commands', async () => {
    ...
    // v3.9.39: /help kini navigator interaktif — daftar command lengkap ada
    // di katalog (dipakai dropdown kategori & view 📖 Semua Command).
    ...
    // v3.9.39: navigator harus membawa dropdown + tombol navigasi.
    assert.ok(replies[0].components?.length >= 2, 'reply harus punya dropdown + tombol');
...
test('help.js: mentions /update-category and /update-product', async () => {
    ...
    const helpHandler = require('../../src/commands/help');
    await helpHandler(mockInteraction);
    // v3.9.39: daftar command pindah ke katalog (navigator interaktif).
    const { HELP_CATEGORIES } = require('../../src/ui/helpCatalog');
