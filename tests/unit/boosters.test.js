...
test('boostHandler: pure embed builders — boost add/remove', () => {
    ...
    assert.match(add.data.title, /BOOST SERVER BARU/);
    ...
    assert.match(add.data.fields.find(f => f.name === '📊 Server').value, /Level Server \*\*2\*\*/);

    const remove = buildBoostRemoveEmbed({ ...member, premiumSinceTimestamp: null }, 1700000000000);
    assert.match(remove.data.title, /BOOST BERAKHIR/);
    assert.match(remove.data.description, /sudah tidak lagi boost/);
...
test('EVENT guildMemberUpdate: premium_since null→date sends boost notification + server log + history', async () => {
    ...
    const boostMsg = sent.find(m => m.embeds?.[0]?.data?.title?.includes('BOOST SERVER BARU'));
    assert.ok(boostMsg, 'notifikasi boost terkirim');
    assert.strictEqual(boostMsg.content, `<@${USER_ID}>`, 'booster is mentioned outside the embed');

    // 2. The server log got a BOOST_ADD entry.
    const logMsg = sent.find(m => m.embeds?.[0]?.data?.title?.includes('Boost Dimulai'));
    assert.ok(logMsg, 'entri server-log BOOST_ADD terkirim');
...
test('EVENT guildMemberUpdate: premium_since date→null notifies removal; no change → silent', async () => {
    ...
    const removal = sent.find(m => m.embeds?.[0]?.data?.title?.includes('BOOST BERAKHIR'));
    assert.ok(removal, 'notifikasi boost berakhir terkirim');
    assert.strictEqual(boostManager.getRecentEvents(GUILD_ID, 1)[0].event, 'remove');

    // No change → nothing sent.
    sent.length = 0;
    await eventHandler.execute(newMember, { ...newMember });
    assert.strictEqual(sent.length, 0, 'tidak ada perubahan premium_since → tanpa notifikasi');
});
...
test('EVENT guildMemberUpdate: booster channel NOT set → console warning names the fix command', async () => {
    ...
    assert.ok(
        warnings.some(w => /channel server-booster BELUM di-set/.test(w) && /\/set-channel server-booster/.test(w)),
...
test('/boosters command: live list from the members cache + recent history + no boosters case', async () => {
    ...
    assert.match(embed.data.title, /SERVER BOOSTER — Boost Test Server/);
    ...
    assert.strictEqual(fields['⭐ Booster Terdaftar'], '2');
    assert.match(fields['📊 Level Server'], /Level 1 \(2 boost\)/);
    assert.match(fields['🕘 Aktivitas Boost Terbaru'], /<@user_alpha>/);
    ...
    assert.match(replies[0].embeds[0].data.description, /belum punya booster aktif/);
});
