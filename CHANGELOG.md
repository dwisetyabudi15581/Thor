...
- 🟢 **+28 unit test (total 644):** `premiumKeys.test.js` — format/unik key, field stok, durasi-sejak-ditukar, single-use atomic, pesan generik semua jalur kegagalan, guild-scoped (list/revoke/redeem), rate limiter (5 kegagalan, window 10 menit, reset sukses, isolasi antar user), stats available terpisah, stok selamat dari cleaner, dan 4 kontrak registry/router (terdaftar, ter-route, public vs admin-gated, opsi command).
