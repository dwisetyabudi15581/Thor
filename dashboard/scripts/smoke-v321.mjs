#!/usr/bin/env node
/**
 * Smoke test v3.21.0 — endpoint Panduan Cepat (panels + verify-panel) pada
 * MOCK DASH API (server tiruan, tanpa bot asli).
 *
 ...
 *   6. POST /guilds/:id/panels             → categoryIds tak cocok → 400
...
console.log(`✅ 0. health OK — guild demo: ${guildId} (channel uji: ${textChannelId})`);
...
check("1. dashboard payload memuat panels", dash1, 200, (d) => Array.isArray(d.panels));

// 2. kosongkan roles.admin (simulasi server belum setup)
...
check("3. panel tiket tanpa roles.admin → 422", noAdmin, 422, (d) => /Role Admin/i.test(d.error));
...
check("8. panel verifikasi tanpa roles.verified → 422", noVerified, 422, (d) => /Role Terverifikasi/i.test(d.error));

// 9. set ulang roles.verified
await call("PUT", "/config", { updates: { "roles.verified": "111111111111111111" } });

// 10. pasang panel verifikasi valid
