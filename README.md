...
- **🚀 Panduan Cepat dari web (v3.21.0)** — mirror kategori 🚀 di `/help`: checklist setup server 6 langkah langsung di dashboard (role admin & verifikasi — **pilih dari daftar atau tempel ID di kolom teks**, kategori & produk + tambah produk cepat, pasang panel tiket & verifikasi ke channel, channel log). Tiap form langsung diterapkan bot ke server; progress bar X/6; server belum setup otomatis mendarat di modul ini.
- **20 modul dashboard** (v3.21.0, dari 11): + **Panduan Cepat**, **Command Manager**, **Custom Command**, **Backup** (buat/restore dari web), **Moderasi** (riwayat warn + tindakan moderator), **Kunci VIP** (beri key produk + auto-expire), **Giveaway** (mulai dari web), **Embed** (builder lengkap + pratinjau), **Poll** (polling interaktif).
